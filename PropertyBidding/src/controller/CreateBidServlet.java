...
public class CreateBidServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {  // get the bidder ID of the selected bidder
			bidderId = Integer.parseInt(request.getParameter("selectedBidder"));
			selectedBidder = bho.searchForBidderById(bidderId);
		} catch (NumberFormatException e) {
			bidderId = null;
			System.out.println("Forgot to select a bidder");
		}
		try {  // get the property ID of the selected property
			propertyId = Integer.parseInt(request.getParameter("selectedProperty"));
			selectedProperty = pho.searchForPropertyById(propertyId);
		} catch (NumberFormatException e) {
			propertyId = null;
		...
		if ((bidderId != null) && (propertyId != null) && (bidAmount != null)) { 
			Bid newBid = new Bid(bidAmount, selectedProperty, selectedBidder);                       
